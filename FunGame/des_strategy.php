...
$sqli = "select id,u_id,s_id,commit_time,commit from strategy_commit where s_id='{$id}'";
$row=fetchOne($sql);
...
<div class="des-info">
    <div class="des-tit">
        <h1><?php echo $row['strategy_tit']?></h1>
    </div>
    <div class="des-summary">
        <h5><?php echo $row['strategy_pre']?></h5>
    </div>
    <?php echo $row['strategy_content']?>

</div>
<div class="commit">
    <form id="addCommentForm" method="post" action="addStrategyCommit.php?id=<?php echo $_REQUEST['id'];?>">
