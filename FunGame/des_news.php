...
$sqli = "select id,u_id,n_id,commit_time,commit from news_commit where n_id='{$id}'";
$row=fetchOne($sql);
...
<div class="des-info">
    <div class="des-tit">
        <h1><?php echo $row['news_tit']?></h1>
    </div>
    <div class="des-summary">
        <h5><?php echo $row['news_pre']?></h5>
    </div>
    <?php echo $row['news_content']?>

</div>
<div class="commit">
    <form id="addCommentForm" method="post" action="addNewsCommit.php?id=<?php echo $_REQUEST['id'];?>">
